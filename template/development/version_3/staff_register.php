...
if ($_SERVER["REQUEST_METHOD"] == "POST") {//checking a super globle to see if the request methord is post to call the page
    ...
    $_POST['role'] = htmlspecialchars($_POST['role'], ENT_QUOTES, 'UTF-8');


    try {

            if (!s_username_check(dbconnect_insert(), $_POST['username'])) {//checks the value returned to see if username id avalible
                if (s_new_user(dbconnect_insert(), $_POST)) {//commits new user to database
                    s_auditor(dbconnect_insert(), getnewstaffid(dbconnect_insert(), $_POST['username']), "reg", "new staff registered");//this logs that a user has registerd and stores in database
            ...
            $_SESSION['s_usermessage'] = "ERROR USER REG FAILED ";//catches any other errors that occer and formatt and  outputs them
        }
...
echo "<p> please enter role above </p>";
echo "<br>";
