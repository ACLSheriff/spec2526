...
$appts = gift_getter(dbconnect_insert());//getting appoiments from database
if(!$appts){//if there are no appoiments it will tell the user
    ...
    echo "<table id='bookings'>";//starts a table for bookings
        ...
        echo "<tr>";
        echo "<td> brand:" . ( $appts['brand']) . "</td>";//using a built in fuction and telling it what format our epoch time should go in for when the apt is
        echo "<td> Made on: " . ( $appts['about']) . "</td>";//using a built in fuction and telling it what format our epoch time should go in for when the apt was made
        echo "<td><input type='hidden' name='giftid' value='".$appts['wishlist_id']."'>
