...
if (!isset($_SESSION['staff_id'])) {//if the user id is not set stops them accessing page for security
...
//this should be here so if there is a use of headers it can be done so the rest of teh code dosnt load so teh headers will work and change page without errors becuse the header has loaded
    ...
    try {
        ...
        if(appt_update(dbconnect_insert(),$_SESSION['apptid'], $epoch_time)){//trys to update appoiment and change in database
            ...
            s_auditor(dbconnect_insert(),$_SESSION['staff_id'],"log", "staff has changed an appointment". $_SESSION['staff_id']);//audits that the user has changed an appoimnet
            header("Location: staff_bookings.php");// sends back to booking page
            exit;
        }else{
...
try {
    ...
    if (!$appt) {
        $_SESSION['usermessage'] = "Error: appointment not found.";
