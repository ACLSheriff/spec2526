...
//this should be here so if there is a use of headers it can be done so the rest of teh code dosnt load so teh headers will work and change page without errors becuse the header has loaded
    ...
    try {
        ...
        $epoch_time = strtotime(time());//converting to epoc time this passing of the veribale is best practice and minimises issues
        if(add_house(dbconnect_insert(), $epoch_time) and add_house_reg(dbconnect_insert(), $_SESSION['user_id'])){//trys to commit the house
